...
const NON_EDUCATIONAL_MIN_TOTAL = 26;
const NON_EDUCATIONAL_MAX_TOTAL = 54;
...
function getInputs() {
  ...
  return {
    ...
    ocr,
    combined,
    audio: finiteScore(els.audioQuality.value, 7),
    video: finiteScore(els.videoQuality.value, 7),
    slides: finiteScore(els.slideReadability.value, 6),
    pace: finiteScore(els.speechPace.value, 8)
  };
}
...
function calculateScores(video = null) {
  ...
  const data = video
    ? {
        ...
        audio: resolveVideoMetric(video, "audio", 7),
        video: resolveVideoMetric(video, "video", 7),
        slides: resolveVideoMetric(video, "slides", 6),
        pace: resolveVideoMetric(video, "pace", 8)
      }
...
function analyzeVideo(video) {
  ...
  const baseTotal = weightedTotal(scores);
  const total = flags.educationalFit.exclude ? nonEducationalPenaltyTotal(flags.educationalFit, baseTotal) : baseTotal;
...
function nonEducationalPenaltyTotal(fit = {}, baseTotal = null) {
  ...
  const baseline = Number.isFinite(baseTotal)
    ? Number(baseTotal)
    : Math.round(30 + educationalSignal * 3 + Math.min(learningEvidence, 4) * 2);

  const raw = (
    baseline * 0.55
    + educationalSignal * 2.0
    + Math.min(learningEvidence, 4) * 1.4
...
function assessEducationalFit(data = {}, segments = [], signals = {}) {
  ...
  const titleEducationHits = regexHits(title, [
    /(^|\s)(урок|уроки|заняти[ея]|лекци[яи]|курс|семинар|вебинар|мастер-?класс|тренинг|инструкци[яи]|руководств[оа]|tutorial|lesson|lecture|course|guide|workshop|webinar|training|class|classes)(\s|$)/i,
    /обуч|учеб|изуч|науч|разбер|объясн|решаем|решени[ея]|практик|тренаж[её]р|гайд|guide|walkthrough|explained|teaching|learning|learn|teach/i,
    /\bhow to\b|как\s+(сделать|решить|настроить|использовать|выучить|понять|работает|работать|приготовить|починить|создать|получить|оформить)\b/i,
    /с\s+нуля|from\s+scratch|для\s+начинающ|beginner|principiantes|débutant|debutant|anfänger|anfanger|principiante/i,
    /lecci[oó]n|curso|aprender|enseñar|ensenar|formation|cours|apprendre|lektion|kurs|lernen|unterricht|lezione|corso|imparare|aula/i
  ]);
  ...
  const formatHits = countHits(text, [
    "обуч", "учеб", "урок", "уроки", "занятие", "курс", "лекция", "семинар", "вебинар", "мастер-класс", "тренинг",
    "научу", "научить", "научиться", "вы научитесь", "изуч", "объясн", "инструкция", "руководство", "гайд", "пошаг",
    "tutorial", "lesson", "lessons", "course", "learning", "learn", "teach", "teaching", "training", "workshop", "webinar",
  ...
  const quickTimePromiseMarkers = countHits(text, [
  ...
  const quickLessonPromiseMarkers = regexHits(text, [
    /за\s+\d+\s+(урок|урока|уроков|заняти[еяй]|lesson|lessons|class|classes|дн[еяй]|недел[юиь]|час[аов]?|месяц)/i
  ]);
  const quickPromiseMarkers = quickTimePromiseMarkers + quickLessonPromiseMarkers;
  ...
  const salesOnlyCourseMention = /курс\s+(стоит|со\s+скид|куп|заказ|полный|прода|оплат)|куп(и|ить|айте)[^.\n]{0,40}курс|скидк[аиу][^.\n]{0,40}курс/i.test(text);
  const hasLectureFormat = /лекци|lecture|chapter|глава|серия|часть|модуль|course|курс/i.test(text) && !salesOnlyCourseMention;
  const hasCourseLikeStructure = hasChapterStructure || hasCaptionStructure || hasSegmentedLearningFlow;
  const hasHowToTitle = /(^|\s)как\s+(сделать|решить|настроить|использовать|выучить|понять|работает|работать|приготовить|починить|создать|получить|оформить)\b/i.test(title);
  const hasFromScratchTitle = /(с\s+нуля|from\s+scratch|для\s+начинающ|beginner)/i.test(title);
  const titleTeachingHits = regexHits(title, [
    /урок|заняти|лекци|курс|семинар|вебинар|мастер-?класс|тренинг|инструкци|руководств|tutorial|lesson|course|lecture|workshop|webinar|training|class|обуч|учеб|науч|изуч|разбор|гайд|guide|how to|explained|walkthrough/i,
    /lecci[oó]n|curso|aprender|enseñar|ensenar|formation|cours|apprendre|lektion|kurs|lernen|unterricht|lezione|corso|imparare|aula/i
  ]);
  ...
  const isCourseOverviewSales = /курс|course/i.test(text) && /обзор|review/i.test(text) && !hasOutcomeSignal && salesPushMarkers >= 1 && !hasMethod;
  ...
  const nonEducational = (score < 2.6 && !hasStrongTitleIntent) ||
    ...
    (motivationalMarkers >= 2 && quickPromiseMarkers >= 2 && salesPushMarkers >= 2 && !hasEducationalSignal) ||
    (salesPushMarkers >= 3 && guaranteeMarkers >= 2 && !hasEducationalSignal) ||
...
function buildRisks(scores, flags, data) {
  ...
  if (flags.educationalFit.exclude) {
    const adjusted = nonEducationalPenaltyTotal(flags.educationalFit, baseTotal);
    risks.push(["high", "Не классифицировано как обучающее видео", `По описанию и содержанию не хватает учебной механики: цели, шагов, практики или проверки понимания. Итог понижен с ${baseTotal} до ${adjusted} (диапазон ${NON_EDUCATIONAL_MIN_TOTAL}-${NON_EDUCATIONAL_MAX_TOTAL} для не-обучающих роликов). ${flags.educationalFit.reasons.join("; ")}.`]);
  }
...
function renderEvidence(scores, flags = null) {
  ...
  if (flags?.educationalFit?.exclude) {
    const adjusted = nonEducationalPenaltyTotal(flags.educationalFit, baseTotal);
    rows.push(["Классификация", `Видео не прошло критерии обучающего формата, поэтому итог понижен с ${baseTotal} до ${adjusted} (${NON_EDUCATIONAL_MIN_TOTAL}-${NON_EDUCATIONAL_MAX_TOTAL} для не-обучающего формата). Причины: ${flags.educationalFit.reasons.join("; ")}.`]);
...
function buildRatingPrompt(video, scores, flags, risks) {
  ...
  const baseTotal = weightedTotal(scores);
  const currentTotal = flags.educationalFit.exclude ? nonEducationalPenaltyTotal(flags.educationalFit, baseTotal) : baseTotal;
  const currentGrade = gradeFor(currentTotal);
  ...
  const sheets = [
    ...
    },
    {
      ...
      rows: [
        ...
        ["Не прошел критерий обучающего формата", flags.educationalFit.exclude],
...
function renderDashboard() {
  ...
  const ranked = analyzed.sort((a, b) => b.total - a.total);
...
function update() {
  ...
  const baseTotal = weightedTotal(scores);
  const total = isExcluded ? nonEducationalPenaltyTotal(flags.educationalFit, baseTotal) : baseTotal;
  const grade = gradeFor(total);
  ...
  els.ratingSummary.textContent = !isExcluded
    ...
    : `Ролик не прошел критерии обучающего формата, поэтому итог понижен до ${total} (диапазон ${NON_EDUCATIONAL_MIN_TOTAL}-${NON_EDUCATIONAL_MAX_TOTAL} для не-обучающих роликов). Причины: ${flags.educationalFit.reasons.join("; ")}.`;
